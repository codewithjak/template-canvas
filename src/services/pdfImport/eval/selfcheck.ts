/**
 * pdfImport/eval/selfcheck.ts
 *
 * Framework-free smoke test for the deterministic pipeline. Builds a synthetic
 * ExtractedDocument with known PDF-space coordinates, runs the pipeline, and
 * asserts geometry/schema correctness. Run with: npx tsx <this file>.
 *
 * This is the stand-in until the pdfjs adapter enables the real
 * template → PDF → extract round-trip (Phase 0 producer half).
 */

import { runDeterministicImport } from '../index';
import { diffTemplates } from './diff';
import type { ExtractedDocument } from '../types';

const A4 = { widthPt: 595.28, heightPt: 841.89 }; // matches PAGE_SIZE_PRESETS.a4
const PX = 96 / 72;
...
// A text run near the TOP of the page (high PDF-y), a horizontal rule, a box.
  ...
  pages: [{
    ...
    primitives: [
      { id: 'p1', source: 'vector', confidence: 1, kind: 'text',
        text: 'Invoice', fontName: 'ABCDEE+Helvetica-Bold', fontSizePt: 18, rotation: 0, color: '#111111',
        rect: { x: 72, y: 800, width: 120, height: 18 } },
      // two runs that should merge into one line block
...
// p2 + p3 merge → one text line "Total: $250"; p1 is its own line.
...
// ── Geometry assertions: PDF bottom-left → canvas top-left ───────────────────
const title = byType('text').find(e => (e as { content: string }).content === 'Invoice')!;
// canvasY = (841.89 - (800+18)) * PX = ~31.85
...
// box at PDF lower-left (72,100,h=80): canvasY = (841.89-180)*PX
...
// ── Round-trip the scorer against itself: identical docs → perfect score ─────
